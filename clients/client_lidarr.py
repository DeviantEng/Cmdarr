...
_SPOTIFY_ARTIST_URL_RE = re.compile(
    r"https?://(?:open\.)?spotify\.com/artist/([a-zA-Z0-9]+)", re.IGNORECASE
)

# Regex to extract Deezer artist ID from URLs like https://www.deezer.com/artist/12345
_DEEZER_ARTIST_URL_RE = re.compile(
    r"https?://(?:www\.)?deezer\.com/(?:[a-z]{2}/)?artist/(\d+)", re.IGNORECASE
)


def _extract_spotify_id_from_links(links: list[dict[str, Any]]) -> str | None:
    """
...
def _extract_deezer_id_from_links(links: list[dict[str, Any]]) -> str | None:
    """
...
class LidarrClient(BaseAPIClient):
    def __init__(self, config):
        headers = {"X-Api-Key": config.LIDARR_API_KEY, "Content-Type": "application/json"}
        ...
        try:
            ...
            if self.cache_enabled and self.cache:
                ...
                if cached_exclusions is not None:
                    self.logger.debug(
                        f"Cache hit for import list exclusions: {len(cached_exclusions)} exclusions"
                    )
    ...
    async def get_all_artists(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for artist in artists_data:
                ...
                if not mbid:
                    self.logger.warning(
                        f"Artist '{name}' has no MBID - this should not happen in Lidarr"
                    )
                    continue
            ...
            if missing_mbid_count > 0:
                self.logger.warning(
                    f"{missing_mbid_count} artists in Lidarr missing MBID (unexpected)"
                )
    ...
    async def add_artist(
        ...
        try:
            ...
            if response:
                ...
                return {"success": False, "error": "No response from Lidarr API"}
