...
class ListenBrainzClient(BaseAPIClient):
    ...
    async def _make_request(
        ...
        return await builder.endpoint(endpoint).params(**(params or {})).execute(self.session)
        ...
        try:
            ...
            if self.cache_enabled and self.cache:
                self.cache.set(
                    ...
                    "listenbrainz",
                    playlists,
                    self.config.get("CACHE_LISTENBRAINZ_TTL_DAYS", 3),
                )
    ...
    async def get_user_recommendation_playlists(self, username: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            self.logger.debug(
                f"Found {len(all_playlists)} recommendation playlists for user: {username}"
            )
    ...
    async def get_curated_playlists(self, username: str) -> dict[str, dict[str, Any] | None]:
        """
        ...
        try:
            ...
            for playlist_wrapper in rec_playlists:
                ...
                if any(term in title for term in ["weekly exploration", "weekly discovery"]):
                    if curated_playlists["weekly_exploration"] is None:
                        ...
                        self.logger.debug(
                            f"Found Weekly Exploration playlist: {playlist.get('title')}"
                        )
            ...
            return {"weekly_exploration": None, "weekly_jams": None, "daily_jams": None}

    async def _get_full_playlist_from_identifier(
        ...
        try:
            ...
            playlist_mbid = (
                playlist_identifier.split("/")[-1]
                if "/" in playlist_identifier
                else playlist_identifier
            )
    ...
    async def get_playlist_details(self, playlist_mbid: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            self.logger.debug(
                f"Retrieved playlist '{playlist.get('title', 'Unknown')}' with {track_count} tracks"
            )
            return playlist
    ...
    async def extract_tracks_from_playlist(self, playlist: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(
                f"Processing {len(playlist_tracks)} tracks from playlist '{playlist.get('title', 'Unknown')}'"
            )
            ...
            for track in playlist_tracks:
                ...
                if "extension" in track:
                    ...
                    self.logger.debug(
                        f"Additional metadata for '{track_title}': {additional_metadata}"
                    )
                ...
                if not track_title or not artist_name:
                    self.logger.debug("Skipping track with missing essential data")
                    continue
                ...
                if album_name:
                    self.logger.debug(
                        f"Extracted track: '{artist_name}' - '{track_title}' - Album: '{album_name}'"
                    )
                else:
                    self.logger.debug(
                        f"Extracted track: '{artist_name}' - '{track_title}' - No album data"
                    )
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            if username and self.cache_enabled and self.cache:
                ...
                if cached_result is not None:
                    self.logger.info(
                        "ListenBrainz API connection test passed - valid cached data available"
                    )
                    return True
