...
class BaseAPIClient(ABC):
    ...
    async def get_api_stats(self) -> dict[str, Any]:
        ...
        if self.cache_enabled and self.cache:
            ...
            client_cache_count = cache_stats.get("cache_counts_by_source", {}).get(
                self.client_name, 0
            )
    ...
    async def _get(
        self, endpoint: str, params: dict[str, str] = None, **kwargs
    ) -> dict[str, Any] | None:
        """Convenience method for GET requests"""
        return await self._make_request(endpoint, params=params, method="GET", **kwargs)

    async def _post(
        self, endpoint: str, params: dict[str, str] = None, json: dict[str, Any] = None, **kwargs
    ) -> dict[str, Any] | None:
