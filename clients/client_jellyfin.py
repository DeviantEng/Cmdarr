...
class JellyfinClient(BaseAPIClient):
    ...
    def get_cache_key(self, library_key: str = None) -> str:
        ...
        if self.base_url is None:
            ...
            base_url = (
                self.base_url.replace("http://", "").replace("https://", "").replace("/", "_")
            )
        if library_key:
    ...
    def _load_cached_library(self) -> None:
        ...
        try:
            ...
            if cached_data:
                ...
                self.logger.info(
                    f"Loaded cached library with {cached_data.get('total_tracks', 0)} tracks"
                )
            else:
    ...
    def _get_sync(self, path: str, params: dict[str, Any] = None) -> dict[str, Any] | None:
    ...
    def _post_sync(
        self, path: str, params: dict[str, Any] = None, json_data: dict[str, Any] = None
    ) -> dict[str, Any] | None:
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            if not system_response:
                self.logger.error(
                    "Failed to connect to Jellyfin server - system info not available"
                )
                return False

            self.logger.info(
                f"Connected to Jellyfin server: {system_response.get('ServerName', 'Unknown')}"
            )

            # Test authentication by getting user info with the configured user ID
            response = self._get_sync(f"/Users/{self.user_id}")
            if response and response.get("Id"):
                self.logger.info("Successfully connected to Jellyfin server")
                return True
            else:
    ...
    def authenticate(self) -> bool:
        ...
        try:
            ...
            response = self._get_sync(f"/Users/{self.user_id}")
            if response and response.get("Id"):
                self.logger.info("Successfully authenticated with Jellyfin server")
                return True
            else:
    ...
    def get_server_info(self) -> dict[str, Any]:
        ...
        try:
            ...
            if response:
                ...
                self.logger.info(
                    f"Connected to Jellyfin server '{server_name}' (version {version})"
                )
                return {
        ...
        try:
            ...
            if self.cache_enabled:
                ...
                if cached_result is not None:
                    self.logger.debug(
                        f"Cache hit for search_tracks: {query}, returning {len(cached_result)} cached tracks"
                    )
            ...
            if item.get("Type") == "Audio":
                track_data = {
                    ...
                    "artist": item.get(
                        ...
                        if item.get("Artists") and len(item.get("Artists", [])) > 0
                        else "Unknown",
                    ),
    ...
    def find_track_by_artist_and_title_sync(
        ...
        try:
                ...
                if track_id:
                    ...
                    for track in tracks:
                        if track.get("id") == track_id:
                            self.logger.debug(
                                f"Found track in cache: '{track['artist']}' - '{track['name']}'"
                            )
                            # Record cache hit
            ...
            self.logger.debug(
                f"Normalized search: '{normalized_artist}' - '{normalized_title}' - '{normalized_album}'"
            )
            ...
            match = self._find_best_match(
                tracks, normalized_artist, normalized_title, artist, title, normalized_album, album
            )
            if match:
                return match
            ...
            artist_words = [w for w in normalized_artist.split() if len(w) > 2][
                :3
            ...
            match = self._find_best_match_relaxed(
                all_tracks,
                normalized_artist,
                normalized_title,
                artist,
            ...
            match = self._find_best_match_ultra_relaxed(
                all_tracks, normalized_title, title, normalized_artist
            )
            if match:
                return match
            ...
            if full_tracks:
                match = self._find_best_match_ultra_relaxed(
                    full_tracks, normalized_title, title, normalized_artist
                )
                if match:
                    self.logger.info(
                        f"Full search match found: '{match['artist']}' - '{match['name']}' for '{artist}' - '{title}'"
                    )
                    return match
            ...
            truncated_title = self._truncate_title_for_search(
                title, 25 - len(artist) - 1
            )  # -1 for space
            if truncated_title != title:
                ...
                if truncated_tracks:
                    match = self._find_best_match_ultra_relaxed(
                        truncated_tracks, normalized_title, title, normalized_artist
                    )
                    if match:
                        self.logger.info(
                            f"Truncated search match found: '{match['artist']}' - '{match['name']}' for '{artist}' - '{title}'"
                        )
                        return match
            ...
            title_only_match = self._try_title_only_search_with_validation(
                title, artist, normalized_artist, normalized_title
            )
            if title_only_match:
            ...
            progressive_match = self._try_progressive_title_word_removal(
                title, artist, normalized_artist, normalized_title
            )
            ...
            self.logger.info("   📋 Search attempts made:")
            self.logger.info(f"     1️⃣ Title search: '{title}' ({len(tracks)} results)")
            self.logger.info(
                f"     2️⃣ Artist search: '{artist}' ({len(artist_tracks) if 'artist_tracks' in locals() else 0} results)"
            )
            self.logger.info(
                f"     3️⃣ Combined search: '{artist} {title}' ({len(combined_tracks) if 'combined_tracks' in locals() else 0} results)"
            )
            ...
            if "full_tracks" in locals():
                self.logger.info(
                    f"     7️⃣ Full search: '{artist} {title}' ({len(full_tracks)} results)"
                )
            ...
            self.logger.info("     🔟 Progressive title word removal: Multiple variants tried")
            self.logger.info(
                "   🔍 All search strategies (1-10) failed to find a match above 25% similarity"
            )
    ...
    def _normalize_text(self, text: str) -> str:
        ...
        normalized = re.sub(
            r"\s*\(.*?(remix|mix|edit|version|remaster|radio|clean|explicit|instrumental|acoustic|live|demo).*?\)\s*",
            " ",
            normalized,
            flags=re.IGNORECASE,
        )
    ...
    def _find_best_match(
        ...
        for track in tracks:
            if (
                track["artist"].lower() == original_artist.lower()
                and track["name"].lower() == original_title.lower()
            ):
                ...
                if original_album and track.get("album"):
                    ...
                    if track_album_norm == normalized_album:
                        ...
                        len(normalized_album) >= 2
                        and len(track_album_norm) >= 2
                        and (
                            normalized_album in track_album_norm
                            or track_album_norm in normalized_album
                        )
                    ):
                        album_bonus = 0.1  # 10% bonus for partial album match
                self.logger.debug(
                    f"Exact match found: '{track['artist']}' - '{track['name']}' (album bonus: {album_bonus:.1%})"
                )
                return track
        ...
        if normalized_artist or normalized_title:
            for track in tracks:
                ...
                if track_artist_norm == normalized_artist and track_title_norm == normalized_title:
                    # Check album match for bonus scoring
                    album_bonus = 0
                    if normalized_album and track.get("album"):
                        ...
                        if track_album_norm == normalized_album:
                            ...
                            album_bonus = 0.1  # 10% bonus for partial album match
                    self.logger.debug(
                        f"Normalized exact match found: '{track['artist']}' - '{track['name']}' (album bonus: {album_bonus:.1%})"
                    )
                    return track
        ...
        for track in tracks:
            ...
            track_title_norm = self._normalize_text(track["name"])
            if (
                len(normalized_artist) >= min_partial_len
                and len(normalized_title) >= min_partial_len
                and len(track_artist_norm) >= min_partial_len
                and len(track_title_norm) >= min_partial_len
                and normalized_artist in track_artist_norm
                and normalized_title in track_title_norm
            ):
                # Check album match for bonus scoring
                album_bonus = 0
                if normalized_album and track.get("album"):
                    ...
                    if track_album_norm == normalized_album:
                        ...
                        len(normalized_album) >= min_partial_len
                        and len(track_album_norm) >= min_partial_len
                        and (
                            normalized_album in track_album_norm
                            or track_album_norm in normalized_album
                        )
                ...
                self.logger.debug(
                    f"Partial match found: '{track['artist']}' - '{track['name']}' (album bonus: {album_bonus:.1%})"
                )
                return track
        ...
        for track in tracks:
            artist_score = self._calculate_similarity(
                normalized_artist, self._normalize_text(track["artist"])
            )
            title_score = self._calculate_similarity(
                normalized_title, self._normalize_text(track["name"])
            )
        ...
        if best_match:
            self.logger.debug(
                f"Fuzzy match found (score: {best_score:.2f}): '{best_match['artist']}' - '{best_match['name']}'"
            )
            return best_match

        return None

    def _find_best_match_relaxed(
        self,
        tracks: list[dict[str, Any]],
        normalized_artist: str,
        normalized_title: str,
        ...
        for track in tracks:
            ...
            title_word_sim = self._calculate_word_order_similarity(
                normalized_title, track_title_norm
            )
            ...
            if combined_score > best_score and combined_score > 0.5:
                ...
                best_match = track
                self.logger.debug(
                    f"Relaxed match candidate (score: {combined_score:.2f}): '{track['artist']}' - '{track['name']}' | Artist sim: {artist_jaccard:.2f}, Title sim: {title_best:.2f}, Album sim: {album_score:.2f}"
                )

        if best_match:
            self.logger.debug(
                f"Relaxed fuzzy match found (score: {best_score:.2f}): '{best_match['artist']}' - '{best_match['name']}'"
            )
            return best_match
    ...
    def _find_best_match_ultra_relaxed(
        self,
        tracks: list[dict[str, Any]],
        normalized_title: str,
        original_title: str,
        ...
        for track in tracks:
            ...
            if (
                len(normalized_title.replace(" ", "")) <= 4
                and len(track_title_norm.replace(" ", "")) <= 4
            ):
                # For very short titles, be more lenient with character matching
                char_overlap = len(
                    set(normalized_title.replace(" ", "")) & set(track_title_norm.replace(" ", ""))
                )
            ...
            if title_score > best_score and title_score > 0.25:
                ...
                best_match = track
                self.logger.debug(
                    f"Ultra-relaxed match candidate (title score: {title_score:.2f}): '{track['artist']}' - '{track['name']}'"
                )

        if best_match:
            self.logger.info(
                f"Ultra-relaxed match found (title score: {best_score:.2f}): '{best_match['artist']}' - '{best_match['name']}' for '{original_title}'"
            )
            return best_match
    ...
    def create_playlist_sync(self, title: str, track_ids: list[str], summary: str = "") -> bool:
        ...
        try:
            ...
            valid_track_ids = [
                str(tid) for tid in track_ids if tid is not None and str(tid).strip()
            ]
            if len(valid_track_ids) != len(track_ids):
                self.logger.warning(
                    f"Filtered out {len(track_ids) - len(valid_track_ids)} invalid values from track_ids"
                )
            ...
            if response.status_code in [200, 201]:
                ...
                if response_data and response_data.get("Id"):
                    ...
                    self.logger.info(
                        f"Successfully created playlist '{title}' with ID {playlist_id}"
                    )
                    ...
                    if False and self.cache_enabled:
                        try:
                            ...
                            self.logger.debug(
                                f"Invalidated playlists cache after creating playlist '{title}' ({cleared_count} entries cleared)"
                            )
    ...
    def get_playlists_sync(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if not playlists_folder_id:
                self.logger.warning(
                    "Could not find Playlists folder, falling back to top-level search"
                )
                params = {"UserId": self.user_id, "IncludeItemTypes": "Playlist"}
            else:
            ...
            if response.status_code == 200:
                ...
                if data and "Items" in data:
                    ...
                    if self.cache_enabled:
                        ...
                        self.cache.set(
                            cache_key, "jellyfin", cache_data, ttl_days=self.get_cache_ttl()
                        )
                ...
                self.logger.warning(
                    f"No playlists found in Jellyfin response (status {response.status_code})"
                )
                return []
    ...
    def find_playlist_by_name_sync(self, playlist_name: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            self.logger.debug(
                f"Searching for playlist '{playlist_name}' in {len(all_playlists)} playlists"
            )
    ...
    def find_all_playlists_by_name_sync(self, playlist_name: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            matches = []
            self.logger.debug(
                f"Searching for ALL playlists named '{playlist_name}' in {len(all_playlists)} playlists"
            )
            for playlist in all_playlists:
                playlist_name_found = playlist.get("Name", "")
                if playlist_name_found == playlist_name:
                    self.logger.debug(
                        f"Found matching playlist: '{playlist_name_found}' (ID: {playlist.get('Id', 'Unknown')})"
                    )
    ...
    def find_playlist_by_name(self, playlist_name: str) -> dict[str, Any] | None:
    ...
    def delete_playlist_sync(self, playlist_id: str) -> bool:
        ...
        try:
            ...
            if response.status_code in [200, 204]:
                ...
                if False and self.cache_enabled:
                    try:
                        cleared_count = self.cache.clear_cache("jellyfin")
                        self.logger.debug(
                            f"Invalidated playlists cache after deleting playlist {playlist_id} ({cleared_count} entries cleared)"
                        )
                    except Exception as cache_error:
                        self.logger.warning(
                            f"Failed to clear cache after deleting playlist {playlist_id}: {cache_error}"
                        )
                return True
            else:
                self.logger.error(
                    f"Failed to delete playlist {playlist_id} - status {response.status_code}"
                )
                return False
        except requests.exceptions.RequestException as e:
            ...
            if hasattr(e, "response"):
                self.logger.debug(
                    f"Exception has response: status_code={getattr(e.response, 'status_code', 'None')}"
                )
    ...
    def get_playlist_tracks_sync(self, playlist_id: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            if response.status_code == 404:
                self.logger.debug(
                    f"Playlist {playlist_id} not found (404), returning empty track list"
                )
                return []
            ...
            if hasattr(e, "response") and e.response and e.response.status_code == 404:
                ...
                # Alternative way to catch 404s if response object isn't available
                self.logger.debug(
                    f"Playlist {playlist_id} not found (404 in error message), returning empty track list"
                )
                return []
            else:
    ...
    def _compare_playlist_tracks(
        self, existing_track_ids: list[str], new_track_ids: list[str]
    ) -> bool:
        """
    ...
    def create_or_update_playlist_sync(
        ...
        try:
            ...
            if existing_playlists:
                self.logger.info(
                    f"Found {len(existing_playlists)} existing playlist(s) with name '{title}', validating content..."
                )
                ...
                for existing_playlist in existing_playlists:
                    existing_track_ids = [
                        track["id"]
                        for track in self.get_playlist_tracks_sync(existing_playlist["Id"])
                    ]
                ...
                if has_existing_tracks:
                    self.logger.info(
                        f"Playlist '{title}' already exists with tracks, keeping existing and deleting duplicates"
                    )
                    # Delete any other duplicates with the same name
                    for other_playlist in existing_playlists:
                        other_track_ids = [
                            track["id"]
                            for track in self.get_playlist_tracks_sync(other_playlist["Id"])
                        ]
                ...
                self.logger.info(
                    f"Playlist '{title}' has no existing tracks, deleting all existing and creating new one"
                )
                for existing_playlist in existing_playlists:
                    self.logger.info(
                        f"Deleting existing playlist '{title}' (ID: {existing_playlist['Id']})"
                    )
        ...
        try:
            ...
            for i, track in enumerate(tracks):
                try:
                    ...
                    self.logger.info(
                        f"[{i + 1}/{len(tracks)}] Searching: '{track['artist']}' - '{track_title}'"
                    )
                    jellyfin_track = self.find_track_by_artist_and_title_sync(
                        track["artist"], track_title, track_album
                    )
                    if jellyfin_track:
                        found_tracks.append(jellyfin_track)
                        self.logger.info(
                            f"[{i + 1}/{len(tracks)}] ✅ '{jellyfin_track['artist']}' - '{jellyfin_track['name']}'"
                        )
                    else:
                        unmatched_tracks.append(f"{track['artist']} - {track_title}")
                        self.logger.info(
                            f"[{i + 1}/{len(tracks)}] ❌ '{track['artist']}' - '{track_title}'"
                        )
                except Exception as e:
                    unmatched_tracks.append(
                        f"{track.get('artist', 'Unknown')} - {track.get('track', track.get('title', 'Unknown'))}"
                    )
            ...
            if success:
                ...
                self.logger.info(
                    f"Successfully synced playlist '{title}': {len(found_tracks)}/{len(tracks)} tracks ({match_rate:.1f}% success rate)"
                )
                return {
                    "success": True,
                    "action": "synced",
        ...
        try:
            ...
            if not library_key:
                ...
                if chosen:
                    ...
                    return {}
            self.logger.info(
                f"Building optimized library cache for Jellyfin library {library_key or 'default'}..."
            )
            ...
            self.logger.info(
                f"Built Jellyfin library cache: {len(all_tracks):,} tracks in {build_time:.1f}s"
            )
    ...
    def _build_optimized_indexes(self, cache_data: dict[str, Any]) -> None:
        ...
        try:
            ...
            for track in tracks:
                ...
                track_name = (
                    str(track_name_raw).lower().strip() if track_name_raw is not None else ""
                )
            ...
            self.logger.debug(
                f"Built indexes: {len(artist_index)} artists, {len(track_index)} tracks"
            )
    ...
    def _fuzzy_match(self, search_term: str, cached_term: str) -> bool:
        ...
        if (
            ...
            and (search_term in cached_term or cached_term in search_term)
        ):
            return True
    ...
    def _try_progressive_title_search(
        self, title: str, artist: str, normalized_artist: str, normalized_title: str
    ) -> dict[str, Any] | None:
            ...
            if len(shortened_title) < len(title):  # Only try if actually shortened
                ...
                if tracks:
                    ...
                    for track in tracks:
                        ...
                        if (
                            self._calculate_similarity(track_artist_normalized, normalized_artist)
                            >= 0.7
                        ):
                            self.logger.info(
                                f"✅ Progressive title match: '{track['artist']}' - '{track['name']}' for '{artist}' - '{title}'"
                            )
                            return track

        return None

    def _try_special_character_variants(
        self,
        title: str,
        artist: str,
        normalized_artist: str,
        ...
        for variant in variants:
            if variant != title:  # Only try if different
                ...
                if tracks:
                    ...
                    if match:
                        self.logger.info(
                            f"✅ Special character variant match: '{match['artist']}' - '{match['name']}' for '{artist}' - '{title}'"
                        )
                        return match

        return None

    def _try_title_only_with_artist_validation(
        self, title: str, artist: str, normalized_artist: str, normalized_title: str
    ) -> dict[str, Any] | None:
        ...
        self.logger.debug(
            f"   Found {len(tracks)} tracks with title '{title}', validating artists..."
        )
        ...
        if best_match and best_score >= 0.6:  # Lower threshold for title-only search
            self.logger.info(
                f"✅ Title-only match: '{best_match['artist']}' - '{best_match['name']}' (artist similarity: {best_score:.2f})"
            )
            return best_match

        return None

    def _try_artist_only_with_title_validation(
        self, title: str, artist: str, normalized_artist: str, normalized_title: str
    ) -> dict[str, Any] | None:
        ...
        self.logger.debug(
            f"   Found {len(tracks)} tracks with artist '{artist}', validating titles..."
        )
        ...
        if best_match and best_score >= 0.6:  # Lower threshold for artist-only search
            self.logger.info(
                f"✅ Artist-only match: '{best_match['artist']}' - '{best_match['name']}' (title similarity: {best_score:.2f})"
            )
            return best_match
    ...
    def _try_title_only_search_with_validation(
        self, title: str, artist: str, normalized_artist: str, normalized_title: str
    ) -> dict[str, Any] | None:
        """Search by title only, then validate artist matches"""
        ...
        if not tracks:
            self.logger.debug("   Title-only search returned 0 results")
            return None
    ...
    def _try_progressive_title_word_removal(
        self, title: str, artist: str, normalized_artist: str, normalized_title: str
    ) -> dict[str, Any] | None:
        ...
        if len(words) <= 1:
            self.logger.debug(
                f"   Title '{title}' has only {len(words)} word(s), skipping progressive removal"
            )
            return None
        ...
        for i in range(len(words) - 1, 0, -1):  # Start with removing 1 word, then 2, etc.
            ...
            if tracks:
                self.logger.debug(
                    f"   Found {len(tracks)} tracks with shortened title '{shortened_title}'"
                )
                ...
                for track in tracks:
                    track_artist_normalized = self._normalize_text(track["artist"])
                    artist_score = self._calculate_similarity(
                        track_artist_normalized, normalized_artist
                    )
                ...
                if best_match and best_score >= 0.7:
                    self.logger.info(
                        f"✅ Progressive word removal match: '{best_match['artist']}' - '{best_match['name']}' (artist similarity: {best_score:.2f})"
                    )
                    return best_match
            else:
