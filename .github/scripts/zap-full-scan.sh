#!/usr/bin/env bash
# ZAP full scan (spider + active scan) against a running container.
# Used as a PR gate - blocks merge if findings exceed configured thresholds.
#
# Usage: ./zap-full-scan.sh <image> [workspace]
...
docker run --rm --network "$NETWORK" -v "${ZAP_WRK}:/zap/wrk:rw" -t ghcr.io/zaproxy/zaproxy:stable \
  zap-full-scan.py -t "$TARGET" -j \
  -c /zap/wrk/zap-baseline.conf -J /zap/wrk/zap-report.json \
  --hook=/zap/wrk/zap_auth_hook.py || exitcode=$?

# Append to GitHub Step Summary when in Actions
if [[ -n "${GITHUB_STEP_SUMMARY:-}" ]]; then
  echo "## ZAP Full Scan Findings" >> "$GITHUB_STEP_SUMMARY"
