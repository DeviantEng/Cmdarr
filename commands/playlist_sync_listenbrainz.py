...
class PlaylistSyncListenBrainzCommand(PlaylistSyncCommand):
    ...
    async def execute(self) -> bool:
        ...
        try:
            self.logger.info(
                f"Starting ListenBrainz playlist sync command: {self.config_json.get('display_name', self.command_name)}"
            )
            ...
            try:
                ...
                if validation_results["overall_status"] == "issues_found":
                    self.logger.warning(
                        f"Found playlist issues: {validation_results['duplicates_found']} duplicates, {validation_results['empty_playlists_found']} empty playlists"
                    )
                    ...
                    if not cleanup_success:
                        self.logger.error(
                            "Failed to clean up playlist issues, continuing with sync..."
                        )
            ...
            if success:
                self.logger.info(
                    f"ListenBrainz playlist sync to {self.target_name} completed successfully"
                )
            else:
    ...
    async def _sync_listenbrainz_playlists(
        self, playlist_types: list[str]
        ...
        if library_cache:
            ...
            if self.target_name.lower() == "plex" and self.config.get(
                "LIBRARY_CACHE_PLEX_ENABLED", False
            ):
                self.target_client._cached_library = library_cache
                self.logger.info(
                    "Plex library cache enabled - using cached library for fast lookups"
                )
            elif self.target_name.lower() == "jellyfin" and self.config.get(
                "LIBRARY_CACHE_JELLYFIN_ENABLED", False
            ):
                self.target_client._cached_library = library_cache
                self.logger.info(
                    "Jellyfin library cache enabled - using cached library for fast lookups"
                )
            else:
                self.logger.info(
                    f"{self.target_name} library cache disabled - using live API searches"
                )
        else:
        ...
        for playlist_key in playlist_types:
            ...
            if not tracks:
                ...
                sync_results[playlist_key] = {
                    ...
                    "playlist_title": playlist_data.get(
                        "title", self._get_display_name(playlist_key)
                    ),
                    "sync_time": 0,
                    "cache_used": False,
                }
            ...
            cached_data = None
            library_key = (
                self.target_client.get_resolved_library_key()
            ...
            self.logger.info(
                f"Syncing '{playlist_key}' ({len(tracks)} tracks) to {self.target_name} as '{target_title}'"
            )

            sync_mode = self.config_json.get("sync_mode", "full")
            if sync_mode == "full":
                result = await self._sync_full(
                    target_title, tracks, description, cached_data, library_key
                )
            else:
                result = await self._sync_additive(
                    target_title, tracks, description, cached_data, library_key
                )
            ...
            if success:
                ...
                self.logger.info(
                    f"Successfully synced '{playlist_key}': {tracks_found}/{tracks_total} tracks ({match_rate:.1f}%) in {playlist_sync_time:.1f}s"
                )
            else:
                self.logger.error(
                    f"Failed to sync '{playlist_key}' after {playlist_sync_time:.1f}s"
                )
        ...
        if library_cache and total_sync_time > 0:
            ...
            self.logger.info(
                f"Library cache performance: {total_sync_time:.1f}s total sync time (estimated {time_saved:.1f}s saved)"
            )
    ...
    def _generate_playlist_description(
        self, playlist_data: dict[str, Any], playlist_key: str
    ) -> str:
        """Generate playlist description"""
        ...
        if original_description:
            ...
            description = type_descriptions.get(playlist_key, "Curated playlist from ListenBrainz")
    ...
    def _build_library_cache_if_enabled(self, target_type: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if not self.config.get(enabled_key, False):
                self.logger.info(
                    f"Library cache building is disabled for {target_type}, skipping cache build"
                )
                return None
    ...
    async def _validate_existing_playlists(self) -> dict[str, Any]:
        ...
        try:
            ...
            if self.target_name == "Plex":
                ...
                # Use Jellyfin API
                all_playlists = (
                    self.target_client.get_playlists_sync()
                    if hasattr(self.target_client, "get_playlists_sync")
                    else []
                )
            ...
            for playlist_name, playlists in playlist_groups.items():
                ...
                if validation["is_duplicate"]:
                    ...
                    validation_results["recommendations"].append(
                        f"Found {len(playlists)} duplicates of '{playlist_name}' - recommend cleanup"
                    )

                if not validation["has_tracks"]:
                    validation_results["empty_playlists_found"] += len(playlists)
                    validation_results["recommendations"].append(
                        f"Found empty playlist(s) '{playlist_name}' - recommend deletion"
                    )
            ...
            if (
                validation_results["duplicates_found"] > 0
                or validation_results["empty_playlists_found"] > 0
            ):
            ...
            self.logger.info(
                f"Validation complete: {validation_results['duplicates_found']} duplicates, {validation_results['empty_playlists_found']} empty playlists"
            )
                    ...
                    if best_playlist:
                        ...
                        for playlist_info in validation["playlists"]:
                            ...
                            if playlist_id != best_playlist_id:
                                self.logger.info(
                                    f"Deleting playlist: {playlist_name} (ID: {playlist_id})"
                                )
                                if hasattr(self.target_client, "delete_playlist_sync"):
                                    ...
                                    if success:
                                        cleanup_count += 1
                                        self.logger.info(
                                            f"Successfully deleted playlist {playlist_id}"
                                        )
                                    else:
                                        self.logger.error(
                                            f"Failed to delete playlist {playlist_id}"
                                        )
                    else:
                        # No best playlist found, delete all
                        self.logger.warning(
                            f"No valid playlist found to keep for {playlist_name}, deleting all"
                        )
                        for playlist_info in validation["playlists"]:
                            playlist_id = playlist_info.get("Id") or playlist_info.get("id")
                            if playlist_id:
    ...
    async def _cleanup_old_playlists(self) -> None:
        ...
        try:
            ...
            if playlists:
                ...
                self.logger.warning(
                    "No playlists found - this might indicate an issue with playlist retrieval"
                )
    ...
    def _group_playlists_by_type_and_date(
        self, playlists: list[dict[str, Any]]
    ) -> dict[str, dict[str, list[dict[str, Any]]]]:
        """Group playlists by type and date for cleanup"""
        groups = {"daily_jams": {}, "weekly_jams": {}, "weekly_exploration": {}}
    ...
    def _cleanup_playlist_type(
        self, playlist_type: str, playlists_by_date: dict[str, list[dict[str, Any]]]
    ) -> int:
        ...
        try:
            ...
            if config_key and command_config and config_key in command_config:
                ...
                try:
                    ...
                    retention_count = 3
                self.logger.info(
                    f"Using command config {config_key}={retention_count} for {playlist_type}"
                )
            else:
                # Fallback to global config - this should not happen for playlist retention!
                self.logger.warning(
                    f"Command config not found for {playlist_type}, using default retention=3"
                )
                retention_count = 3
            ...
            if deleted_count > 0:
                self.logger.info(
                    f"Cleaned up {deleted_count} old {playlist_type} playlists (kept {len(playlists_to_keep)} most recent)"
                )
    ...
    def _log_final_statistics(self):
        ...
        if stats["cache_used"]:
            ...
            if total_cache_ops > 0:
                ...
                self.logger.info("Library Cache Performance:")
                self.logger.info(
                    f"  Cache Hit Rate: {cache_hit_rate:.1f}% ({cache_perf['cache_hits']}/{total_cache_ops})"
                )
                ...
                if stats["total_sync_time"] > 0:
                    estimated_without_cache = (
                        stats["total_sync_time"] * 6
                    )  # Conservative 6x estimate
                    estimated_savings = estimated_without_cache - stats["total_sync_time"]
                    self.logger.info(
                        f"  Estimated Time Saved: {estimated_savings:.1f}s ({estimated_without_cache:.1f}s → {stats['total_sync_time']:.1f}s)"
                    )
        else:
        ...
        for _playlist_key, result in stats["sync_results"].items():
            status = "✅" if result["success"] else "❌"
            cache_indicator = "🚀" if result.get("cache_used") else "🐌"
            sync_time = result.get("sync_time", 0)
            self.logger.info(
                f"  {status} {cache_indicator} {result['playlist_title']}: {result['tracks_found']}/{result['tracks_total']} tracks ({sync_time:.1f}s)"
            )
    ...
    async def _discover_missing_artists(self, sync_results: dict[str, dict[str, Any]]):
        ...
        try:
            ...
            for playlist_key, result in sync_results.items():
                self.logger.info(
                    f"Processing playlist {playlist_key}: success={result.get('success', False)}"
                )
                if result["success"]:
                    ...
                    unmatched_tracks = result.get("unmatched_tracks", [])
                    self.logger.info(
                        f"Playlist {playlist_key} has {len(unmatched_tracks)} unmatched tracks"
                    )
                    ...
                    for track_string in unmatched_tracks:
                        if " - " in track_string:
                            ...
                            self.logger.warning(
                                f"Unmatched track string doesn't contain ' - ': {track_string}"
                            )
                else:
                    self.logger.warning(
                        f"Playlist {playlist_key} failed, skipping unmatched tracks"
                    )

            self.logger.info(f"Total tracks processed: {total_tracks}, matched: {matched_tracks}")
            self.logger.info(
                f"Found {len(unmatched_artists)} unique unmatched artists: {list(unmatched_artists)}"
            )
            ...
            musicbrainz_client = (
                MusicBrainzClient(self.config) if self.config.MUSICBRAINZ_ENABLED else None
            )
    ...
    async def _save_discovered_artists(self, discovered_artists: list[dict[str, Any]]):
        ...
        try:
            ...
            self.logger.info(
                f"Saved {len(new_artists)} new artists to unified discovery file: {discovery_file}"
            )
    ...
    def get_output_summary(self) -> str:
        ...
        summary_parts = []
        summary_parts.append("Source: ListenBrainz")
        ...
        summary_parts.append(
            f"Sync Mode: {self.config_json.get('sync_mode', 'full').replace('_', ' ').title()}"
        )

        if self.last_run_stats["overall_success"]:
            summary_parts.append("Status: Success")
            summary_parts.append(
                f"Playlists Synced: {self.last_run_stats['successful_syncs']}/{self.last_run_stats['total_playlists_configured']}"
            )
            summary_parts.append(
                f"Tracks Found: {self.last_run_stats['total_tracks_found']}/{self.last_run_stats['total_tracks_attempted']}"
            )
            if self.last_run_stats["total_tracks_attempted"] > 0:
                success_rate = (
                    self.last_run_stats["total_tracks_found"]
                    / self.last_run_stats["total_tracks_attempted"]
                    * 100
                )
            ...
            summary_parts.append(
                f"Error: {self.last_run_stats.get('message', 'An unknown error occurred.')}"
            )
