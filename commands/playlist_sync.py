...
class PlaylistSyncCommand(BaseCommand):
    ...
    async def _discover_and_add_artists(
        self, tracks: list[dict[str, Any]], cached_data: dict[str, Any] | None
    ) -> dict[str, Any]:
                ...
                for artist_name in unique_artists:
                    try:
                        ...
                        if not mbid_result or not mbid_result.get("mbid"):
                            ...
                            artists_skipped += 1
                            skipped_artists.append(
                                {"name": artist_name, "reason": "No MBID found in MusicBrainz"}
                            )
                            continue
                        ...
                        artists_failed += 1
                        failed_artists.append({"name": artist_name, "error": str(e)})
    ...
    async def _sync_full(
        ...
        try:
            ...
            if result.get("success"):
                ...
                self.logger.info(
                    f"Full sync completed: {action}, {found_tracks}/{total_tracks} tracks matched"
                )
                return result
            else:
    ...
    async def _sync_additive(
        ...
        try:
            ...
            if not existing_playlist:
                ...
                self.logger.info(
                    f"Playlist '{playlist_title}' doesn't exist, creating with all tracks"
                )
                return await self._sync_full(
                    playlist_title, tracks, summary, cached_data, library_key
                )

            # Get existing tracks
            playlist_rating_key = existing_playlist.get("ratingKey") or existing_playlist.get("Id")
            existing_track_keys = self.target_client.get_playlist_track_rating_keys(
                playlist_rating_key
            )
