...
class DiscoveryLastfmCommand(BaseCommand):
    ...
    async def _discover_similar_artists(self) -> tuple[list[dict[str, Any]], list[str]]:
        ...
        similar_per_artist = max(
            1, int(cfg.get("similar_per_artist") or cfg.get("similar_count", 1))
        )
        cooldown_days = max(1, int(cfg.get("artist_cooldown_days", 30)))
        ...
        if recently_queried:
            self.logger.info(
                f"Excluding {len(recently_queried)} artists queried within last {cooldown_days} days"
            )
        ...
        if len(artist_items) < artists_to_query:
            if artist_items:
                self.logger.info(
                    f"Only {len(artist_items)} artists available (cooldown), using all"
                )
            else:
    ...
    async def _fetch_lastfm_similar_artists(
        ...
        # Log initial results
        self.logger.info(
            f"{lastfm_mbid_success} artists processed successfully, {len(failed_lookups)} failed lookups"
        )
    ...
    def _log_validation_calls(self, failed_lookups: list[tuple]):
        ...
        self.logger.debug(
            f"Failed MBID lookups - validation calls for {len(failed_lookups)} artists:"
        )
        for mbid, artist_name in failed_lookups:
            self.logger.debug(f'  Artist: "{artist_name}" MBID: {mbid}')
            self.logger.debug(
                f"    Check existence: http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&mbid={mbid}&api_key={{API_KEY}}&format=json"
            )
            self.logger.debug(
