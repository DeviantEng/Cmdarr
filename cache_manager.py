...
class CacheManager:
    ...
    def get(self, cache_key: str, source: str) -> dict[str, Any] | None:
            ...
            with self.db_manager.get_cache_session_context() as session:
                ...
                if existing:
                    ...
                    cache_entry = CacheEntry(
                        cache_key=cache_key, source=source, data=data, expires_at=expires_at
                    )
                    session.add(cache_entry)

                session.commit()
                self.logger.debug(
                    f"Cached: {source}:{cache_key} (expires: {expires_at.strftime('%Y-%m-%d')})"
                )
    ...
    def mark_failed_lookup(
        self, cache_key: str, source: str, error_reason: str, ttl_days: int
    ) -> None:
        ...
        try:
            ...
            with self.db_manager.get_cache_session_context() as session:
                ...
                existing = (
                    session.query(FailedLookup)
                    .filter(FailedLookup.cache_key == cache_key, FailedLookup.source == source)
                    .first()
                )
    ...
    def cleanup_expired(self) -> int:
        ...
        try:
            with self.db_manager.get_cache_session_context() as session:
                ...
                expired_failures = (
                    session.query(FailedLookup)
                    .filter(FailedLookup.expires_at <= datetime.utcnow())
                    .delete()
                )
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        try:
            with self.db_manager.get_cache_session_context() as session:
                ...
                cache_counts = {}
                for source, count in (
                    session.query(CacheEntry.source, session.func.count(CacheEntry.id))
                    .filter(CacheEntry.expires_at > datetime.utcnow())
                    .group_by(CacheEntry.source)
                    .all()
                ):
    ...
    def clear_cache(self, source: str | None = None) -> int:
        ...
        try:
            with self.db_manager.get_cache_session_context() as session:
                if source:
                    cleared_cache = (
                        session.query(CacheEntry).filter(CacheEntry.source == source).delete()
                    )
                    cleared_failures = (
                        session.query(FailedLookup).filter(FailedLookup.source == source).delete()
                    )
                else:
