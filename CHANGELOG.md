...
- **Modern *arr UI — settings routing**: Arr mode uses `/settings/:section` and `/system/*` routes (with redirects from legacy `/config` and `/status` bookmarks); header health indicator reflects `/health`.
