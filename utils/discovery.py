...
class DiscoveryUtils:
    ...
    def apply_random_sampling(
        self, candidates: list[dict[str, Any]], limit: int, command_name: str
        ...
        self.logger.info(
            f"Applied random sampling for {command_name}: {len(sampled)} of {len(candidates)} artists (variety boost)"
        )

        return sampled, limited_count, True

    async def process_artists_through_musicbrainz(
        self,
        artists_without_mbids: list[dict[str, Any]],
        existing_mbids: set[str],
        existing_names: set[str],
        excluded_mbids: set[str],
        source_name: str,
            ...
            try:
                ...
                if mb_result:
                    ...
                    if not should_include:
                        ...
                        self.logger.debug(
                            f"MusicBrainz match for '{artist_name}' filtered: {reason}"
                        )
                        continue
                    ...
                    if any(ra["MusicBrainzId"] == mbid for ra in recovered_artists):
                        self.logger.debug(
                            f"MusicBrainz match for '{artist_name}' already recovered: {mbid}"
                        )
                        continue
                    ...
                    self.logger.debug(
                        f"Recovered '{artist_name}' -> '{mb_name}' (MBID: {mbid}, similarity: {similarity:.3f})"
                    )
                else:
        ...
        self.logger.info(
            f"MusicBrainz processing complete: {success_count}/{processed_count} artists recovered"
        )
        if excluded_during_recovery > 0:
            self.logger.info(
                f"✅ Blocked {excluded_during_recovery} excluded artists during MusicBrainz recovery"
            )

        return recovered_artists

    def deduplicate_by_mbid(
        self, artists: list[dict[str, Any]], score_field: str = "match"
    ) -> list[dict[str, Any]]:
        """
    ...
    def log_filtering_statistics(self, command_name: str, stats: dict[str, Any]):
        ...
        if "filtered_already_in_lidarr" in stats:
            self.logger.info(
                f"Filtered - Already in Lidarr:         {stats['filtered_already_in_lidarr']:,}"
            )
        if "filtered_in_exclusions" in stats:
            self.logger.info(
                f"Filtered - Import List Exclusions:    {stats['filtered_in_exclusions']:,}"
            )
        if "filtered_low_score" in stats:
            self.logger.info(
                f"Filtered - Low Match Score:           {stats['filtered_low_score']:,}"
            )
        ...
        if "musicbrainz_recovered" in stats:
            self.logger.info(
                f"MusicBrainz Recovered:                {stats['musicbrainz_recovered']:,}"
            )
        ...
        if exclusions_blocked > 0:
            ...
            self.logger.info(
                f"Exclusion Filter Impact:              {impact:.1f}% of candidates blocked"
            )
            self.logger.info(
                f"✅ Exclusion filtering prevented {exclusions_blocked} unwanted recommendations"
            )

        if stats.get("random_sampling_applied"):
            self.logger.info("✅ Random sampling applied for discovery variety")
        ...
        if final_count > 0:
            self.logger.info(
                f"🎯 SUCCESS: {final_count} new artists discovered and ready for import"
            )
        elif total_candidates > 0:
            filtered_total = stats.get("filtered_already_in_lidarr", 0) + stats.get(
                "filtered_in_exclusions", 0
            )
            if filtered_total == total_candidates:
                self.logger.info(
                    f"🎯 SUCCESS: All {total_candidates} discovered artists are already in Lidarr or excluded"
                )
                self.logger.info("   This indicates your music library is well-curated!")
            else:
                self.logger.info(
                    f"🎯 SUCCESS: Discovery completed - no new artists found from {total_candidates} candidates"
                )
        else:
    ...
    def create_artist_entry(self, mbid: str, name: str, source: str, **kwargs) -> dict[str, Any]:
        ...
        """
        entry = {"MusicBrainzId": mbid, "ArtistName": name, "source": source}
