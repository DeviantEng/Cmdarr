...
class LibraryCacheManager:
    ...
    def __init__(self, config):
        ...
        if hasattr(self, "client_stats") and self.client_stats:
            ...
            self.client_stats = {
                "plex": {"cache_hits": 0, "cache_misses": 0, "last_used": None, "hit_rate": 0.0},
                "jellyfin": {
                    "cache_hits": 0,
                    "cache_misses": 0,
                    "last_used": None,
    ...
    def register_client(self, client_type: str, client_instance) -> None:
        ...
        if client_type not in self.client_stats:
            self.logger.warning(
                f"Unsupported client type '{client_type}', stats may not be tracked properly"
            )
        else:
            self.logger.info(
                f"Client '{client_type}' registered with stats: {self.client_stats[client_type]}"
            )

    def get_library_cache(self, client_type: str, library_key: str = None) -> dict[str, Any] | None:
        """
    ...
    def get_library_cache_metadata(self, client_type: str) -> dict[str, Any] | None:
        """
    ...
    def _get_from_sqlite(
        self, cache_key: str, client_type: str, library_key: str
    ) -> dict[str, Any] | None:
        ...
        try:
            with self.db_manager.get_cache_session_context() as session:
                ...
                if cache_entry:
                    ...
                    self.logger.debug(
                        f"Retrieved {cache_entry.track_count:,} tracks from SQLite cache (created: {cache_entry.created_at})"
                    )
    ...
    def _build_and_store_cache(
        self, client, client_type: str, library_key: str, cache_key: str
    ) -> dict[str, Any] | None:
        ...
        try:
            ...
            # Store in SQLite
            self._store_in_sqlite(
                cache_key, client_type, library_key, cache_data, track_count, ttl_days
            )
            ...
            self.logger.info(
                f"Built and cached {track_count:,} tracks for {client_type}:{library_key}"
            )
            return cache_data
    ...
    def _store_in_sqlite(
        ...
        try:
            ...
            with self.db_manager.get_cache_session_context() as session:
                ...
                existing = (
                    session.query(LibraryCache).filter(LibraryCache.cache_key == cache_key).first()
                )
    ...
    def verify_and_refresh_cache(
        self, client_type: str, library_key: str = None, sample_track_keys: list[str] = None
    ) -> bool:
        """
        ...
        if sample_track_keys:
            ...
            if stale_count > len(sample_track_keys) * 0.2:
                self.logger.info(
                    f"Cache appears stale ({stale_count}/{len(sample_track_keys)} tracks missing), refreshing..."
                )
    ...
    def _invalidate_sqlite_cache(self, cache_key: str) -> None:
        ...
        try:
            with self.db_manager.get_cache_session_context() as session:
                session.query(LibraryCache).filter(LibraryCache.cache_key == cache_key).delete()
                session.commit()
    ...
    def invalidate_cache(self, client_type: str, library_key: str = None) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Successfully invalidated cache for {client_type} (library: {library_key or 'default'})"
            )
    ...
    def cleanup_expired_cache(self) -> int:
        ...
        try:
            with self.db_manager.get_cache_session_context() as session:
                expired_count = (
                    session.query(LibraryCache)
                    .filter(LibraryCache.expires_at <= datetime.utcnow())
                    .delete()
                )
    ...
    def get_cache_stats(self) -> dict[str, Any]:
        ...
        try:
            with self.db_manager.get_cache_session_context() as session:
                ...
                cache_counts = {}
                for client_type, count in (
                    session.query(LibraryCache.client_type, func.count(LibraryCache.id))
                    .group_by(LibraryCache.client_type)
                    .all()
                ):
                ...
                return {
                    ...
                    "global_hit_rate": self.global_stats["total_cache_hits"]
                    / max(
                        1,
                    ...
                    "last_cache_build": self.global_stats["last_cache_build"].isoformat()
                    if self.global_stats["last_cache_build"]
                    else None,
                }
    ...
    def set_library_cache(
        self, client_type: str, cache_data: dict[str, Any], library_key: str = None
    ) -> None:
        ...
        try:
            with self.db_manager.get_cache_session_context() as session:
                if client_type:
                    cleared_count = (
                        session.query(LibraryCache)
                        .filter(LibraryCache.client_type == client_type)
                        .delete()
                    )
                else:
                ...
                if cleared_count > 0:
                    self.logger.info(
                        f"Cleared {cleared_count} library cache entries"
    ...
    def record_cache_hit(self, client_type: str) -> None:
        ...
        total_requests = (
            self.client_stats[client_type]["cache_hits"]
            + self.client_stats[client_type]["cache_misses"]
        )
        self.client_stats[client_type]["hit_rate"] = self.client_stats[client_type][
            "cache_hits"
        ] / max(1, total_requests)

        self.logger.debug(
            f"Cache hit recorded for '{client_type}': {self.client_stats[client_type]['cache_hits']} hits"
        )
    ...
    def record_cache_miss(self, client_type: str) -> None:
        ...
        self.logger.debug(
            f"Cache miss recorded for '{client_type}': {self.client_stats[client_type]['cache_misses']} misses"
        )

    def get_client_stats(self, client_type: str) -> dict[str, Any]:
        """Get cache statistics for a specific client"""
        self.logger.debug(
            f"Getting client stats for '{client_type}', available clients: {list(self.client_stats.keys())}"
        )

        if client_type not in self.client_stats:
            self.logger.debug(
                f"Client '{client_type}' not found in client_stats, returning default stats"
            )
            return {"cache_hits": 0, "cache_misses": 0, "last_used": None, "hit_rate": 0.0}
...
_library_cache_manager: LibraryCacheManager | None = None
