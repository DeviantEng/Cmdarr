...
class CacheClient:
    ...
    def __init__(self, client_name: str, config: dict[str, Any]):
        ...
        self.library_cache_enabled = config.get(
            f"LIBRARY_CACHE_{client_name.upper()}_ENABLED", False
        )
    ...
    def log_cache_performance(self, operation_name: str, start_time: float) -> None:
        ...
        try:
            ...
            if stats:
                ...
                self.logger.info(
                    f"Cache performance for {operation_name}: {duration:.2f}s, {total_ops} operations, {hit_rate:.1%} hit rate"
                )
        except Exception as e:
