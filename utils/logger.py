                ...
                if len(parts) >= 2:
                    try:
                        ...
                        if 200 <= status_code < 300:
                            ...
                            if any(
                                routine in message
                                for routine in ["/health", "/status", "/static/", "GET / "]
                            ):
...
_UVICORN_QUIET_ENDPOINTS = (
    ...
    "/api/commands/",  # command list and related polls
    "/static/",
)
...
class CmdarrLogger:
    ...
    def setup_logging(cls, config) -> None:
        ...
        logger.info(
            f"Logging configured - Level: {config.LOG_LEVEL}, Retention: {config.LOG_RETENTION_DAYS} days"
        )
